...
create type family_status as enum ('Single', 'Married', 'Divorced', 'Window', 'Separated', 'Other');
...
alter table customers add column marital_status varchar;
...
insert into customers (first_name, last_name, email, date_of_birth, gender, nationality, marital_status) values ('Gloria', 'Powrie', 'gpowriem@msn.com', '1986-11-19', 'Female', 'Malay', 'Window');
...
insert into customers (first_name, last_name, email, date_of_birth, gender, nationality, marital_status) values ('Kailey', 'Gate', 'kgatep@lycos.com', '1987-04-21', 'Female', 'Malayalam', 'Window');
...
select o.first_name, h.* from owners o join houses h on o.id = h.owner_id
...
select a.*, h.* from addresses a join houses h on a.id = h.address_id where a.id in (5,6,7,8,9,10);

-- - 4. Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.

select h.*, o.first_name as owner_name, c.first_name as customer_name, a.name as agent_name from rent_info
join houses h on rent_info.house_id = h.id
join owners o on rent_info.owner_id = o.id
join customers c on rent_info.customer_id = c.id
join agencies a on a.id = rent_info.agency_id;

-- - 5. Клиенттердин 10-катарынан баштап 1999-жылдан кийин туулган 15 клиентти чыгар.

select * from customers  where date_of_birth > '1999-01-01' order by date_of_birth  offset 10 limit 15;

-- - 6. Рейтинги боюнча уйлорду сорттоп, уйлордун тайптарын, рейтингин жана уйдун ээлерин чыгар. (asc and desc)

select h.house_type, h.rating, o.first_name as owner_name from houses h

join owners o on o.id = h.owner_id order by rating  ;

select h.house_type, h.rating, o.first_name as owner_name from houses h

join owners o on o.id = h.owner_id order by rating desc ;

-- - 7. Уйлордун арасынан квартиралардын (apartment) санын жана алардын баасынын суммасын чыгар.
select h.house_Type, count(*),sum(price)  from houses h where house_Type = 'Apartment' group by h.house_Type;
...
select a.name as agency_name, a.address_id as agency_address, h.id as house_id, h.address_id as house_address,  h.house_type
from agencies a join houses h on a.address_id = h.address_id where a.name = 'My House';
-- - 9. Уйлордун арасынан мебели бар уйлорду, уйдун ээсин жана уйдун адрессин чыгар.
select o.first_name as owner_name, h.address_id as house_address, h.furniture from houses h
join owners o on o.id = h.owner_id where furniture = true;
-- - 10.Кленти жок уйлордун баарын жана анын адрессин жана ал уйлор кайсыл агентсвога тийешелуу экенин чыгар.
select h.id, h.address_id, h.house_type, a.name as agency_name, a.address_id as agency_address from houses h
...
select c.nationality, count(c.id) as total_customers from customers c group by c.nationality;
...
select h.id, h.address_id, h.house_type from houses h
...
select h.*, a.*, o.* from houses h join addresses a on h.address_id = a.id join owners o on h.owner_id = o.id
order by h.room desc limit 1;
-- - 19.Бишкекте жайгашкан уйлорду жана клиентерин кошо чыгарыныз.
select h.*, c.first_name as customer_name from houses h
...
select h.*, r.check_in, r.check_out, (r.check_out::date - r.check_in::date) as days from rent_info r
join houses h on r.house_id = h.id order by days desc limit 1;
-- - 22.Эн кымбат уйду жана анын ээсин чыгарыныз.
select h.id as house_id, h.price, h.description,o.first_name,o.last_name from houses h
join owners o on h.owner_id = o.id
order by h.price desc limit 1;
-- - 23.Бир региондо жайгашкан баардык агентстволорду чыгарыныз
select a.*, a2.region from agencies a  join addresses a2 on a2.id = a.address_id ;
...
select o.*, h.*, h.address_id from owners o join houses h on o.id = h.owner_id
where o.date_of_birth = (select round(avg(o.date_of_birth)) from owners);
